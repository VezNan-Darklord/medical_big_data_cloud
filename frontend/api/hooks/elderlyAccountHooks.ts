...
export function useListElderlyAccountsQuery(params: { pageSize?: number } = {}) {
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => medical.elderlyAccount.listElderlyAccounts(undefined, pageParam, pageSize),
...
export function useResetElderlyPasswordMutation() {
  return useMutation({ mutationFn: async ({ id, newPassword }: { id: string; newPassword: string }) => medical.elderlyAccount.resetElderlyPassword(id, { newPassword }), mutationKey: ['resetElderlyPassword'] })
}

export function useUpdateElderlyAccountStatusMutation() {
  return useMutation({ mutationFn: async ({ id, ...req }: UserStatusUpdateRequest & { id: string }) => medical.elderlyAccount.updateElderlyAccountStatus(id, req), mutationKey: ['updateElderlyAccountStatus'] })
}
