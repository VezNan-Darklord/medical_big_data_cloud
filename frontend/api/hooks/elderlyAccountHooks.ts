...
export function useListElderlyAccountsQuery(params: { pageSize?: number } = {}) {
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 1 }) => medical.elderlyAccount.listElderlyAccounts(pageParam, pageSize),
...
export function useUpdateElderlyAccountStatusMutation() {
  return useMutation({ mutationFn: async ({ id, ...req }: StatusRequest & { id: string }) => medical.elderlyAccount.updateElderlyAccountStatus(id, req), mutationKey: ['updateElderlyAccountStatus'] })
}
