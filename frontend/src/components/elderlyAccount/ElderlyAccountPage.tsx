...
import { PlusOutlined, ReloadOutlined, StopOutlined, CheckCircleOutlined } from '@ant-design/icons'
import {
  useListElderlyAccountsQuery,
  useCreateElderlyAccountMutation,
  useResetElderlyPasswordMutation,
  useUpdateElderlyAccountStatusMutation,
} from '../../../api/hooks/elderlyAccountHooks'
...
function AccountCard({ account, onResetPassword, onToggleStatus }: UserCardProps) {
    ...
    <Card className="overflow-hidden rounded-2xl border border-slate-200/70 shadow-sm transition hover:shadow-md" styles={{ body: { padding: 20 } }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className={`flex h-12 w-12 items-center justify-center rounded-xl text-lg font-semibold ${isEnabled ? 'bg-cyan-50 text-cyan-700' : 'bg-slate-100 text-slate-400'}`}>
...
function CreateAccountModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const createMutation = useCreateElderlyAccountMutation()

  return (
    <PopWindow open={open} onClose={onClose} title="创建老人账号" width={520}>
...
export default function ElderlyAccountPage() {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } = useListElderlyAccountsQuery({
    status: statusFilter as 'enabled' | 'disabled' | undefined,
    pageSize: 20,
  })
  const toggleStatusMutation = useUpdateElderlyAccountStatusMutation()
  ...
  const handleToggleStatus = (id: string, status: 'enabled' | 'disabled', name: string) => {
    toggleStatusMutation.mutate({ id, status }, {
      onSuccess: () => message.success(`${name} 已${status === 'enabled' ? '启用' : '禁用'}`),
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-[28px] border border-slate-200/80 bg-white px-6 py-5 shadow-sm">
        <div>
          <div className="text-2xl font-semibold text-slate-900">老人账户管理</div>
