  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-[28px] border border-slate-200/80 bg-white px-6 py-5 shadow-sm">
        ...
        {canManage && <Button type="primary" size="large" icon={<PlusOutlined />} onClick={() => setCreateOpen(true)}>添加人群</Button>}
      </div>
      {isLoading ? <Skeleton active paragraph={{ rows: 6 }} /> : (
        <><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{allItems.map(i => <PopulationCard key={i.id} item={i} canDelete={canDelete} canManage={canManage} onEdit={setEditItem} />)}</div><div ref={sentinelRef} className="h-px" />{isFetchingNextPage && <div className="text-center"><Spin /></div>}</>
      )}
      {canManage && <CreateModal open={createOpen} onClose={() => setCreateOpen(false)} />}
      {canManage && <EditModal open={!!editItem} item={editItem} onClose={() => setEditItem(null)} />}
    </div>
  )
}
