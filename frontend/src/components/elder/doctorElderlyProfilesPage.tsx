...
import { Button, Card, Form, Input, InputNumber, Select, DatePicker, Tag, message, Skeleton, Drawer, Descriptions } from 'antd'
import { PlusOutlined, ReloadOutlined } from '@ant-design/icons'
import { useListElderlyProfilesQuery, useCreateElderlyProfileMutation, useGetElderlyProfileQuery } from '../../../api/hooks/elderlyProfileHooks'
...
function ProfileCard({ profile, onResetPassword, onViewDetail }: { profile: ElderlyProfile; onResetPassword: (id: string, name: string) => void; onViewDetail: (id: string) => void }) {
  return (
      ...
      {isLoading ? <Skeleton active paragraph={{ rows: 8 }} /> :
       !profile ? <div className="text-center text-slate-400 py-10">未找到档案</div> : (
        <div className="space-y-4">
          ...
          <div className="text-xs text-slate-400">创建于 {profile.createdAt} · 更新于 {profile.updatedAt}</div>
        </div>
      )}
    </Drawer>
  )
    ...
    <div className="space-y-6">
      ...
      {isLoading ? <Skeleton active paragraph={{ rows: 6 }} /> : (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {allProfiles.map((p) => <ProfileCard key={p.id} profile={p} onViewDetail={setDetailId} onResetPassword={(id, name) => setResetPwdTarget({ id, name })} />)}
        </div>
      )}
